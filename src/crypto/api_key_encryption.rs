...
use fluxencrypt::keys::{KeyPair, PrivateKey, PublicKey};
...
fn get_or_generate_keys() -> Result<(PublicKey, PrivateKey), FluxError> {
    ...
    if env::var(FLUX_PUBLIC_KEY_ENV).is_ok() && env::var(FLUX_PRIVATE_KEY_ENV).is_ok() {
        ...
        eprintln!("========================================\n");
        eprintln!("export {}=\"{}\"", FLUX_PUBLIC_KEY_ENV, public_pem_str);
        eprintln!("\nexport {}=\"{}\"", FLUX_PRIVATE_KEY_ENV, private_pem_str);
        eprintln!("\n========================================");
...
mod tests {
    ...
    fn test_encrypt_decrypt_api_key() {
        ...
        // Store PEM strings directly (not base64 encoded) for tests
        env::set_var(FLUX_PUBLIC_KEY_ENV, keypair.public_key().to_pem().unwrap());
        env::set_var(
            FLUX_PRIVATE_KEY_ENV,
            keypair.private_key().to_pem().unwrap(),
        );
    ...
    fn test_decrypt_with_wrong_keys() {
        ...
        env::set_var(FLUX_PUBLIC_KEY_ENV, keypair1.public_key().to_pem().unwrap());
        env::set_var(
            FLUX_PRIVATE_KEY_ENV,
            keypair1.private_key().to_pem().unwrap(),
        );
        ...
        env::set_var(FLUX_PUBLIC_KEY_ENV, keypair2.public_key().to_pem().unwrap());
        env::set_var(
            FLUX_PRIVATE_KEY_ENV,
            keypair2.private_key().to_pem().unwrap(),
        );
