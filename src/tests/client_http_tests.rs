...
mod client_http_operations {
    ...
    async fn test_client_post_success() {
        ...
        with_api_key(mock_post("/analyses"), "test_api_key_123")
    ...
    async fn test_client_put_success() {
        ...
        with_api_key(mock_put("/files/test/votes"), "test_api_key_123")
            .and(body_json(&request_body))
            .respond_with(response)
            .mount(mock_server)
            .await;
    ...
    async fn test_client_delete_success() {
        ...
        with_api_key(mock_delete("/files/test/comments/123"), "test_api_key_123")
            .respond_with(response)
            .mount(mock_server)
            .await;
